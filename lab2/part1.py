...
from __future__ import annotations  #helps with type hints ##He says we don't need...
from tkinter import *
...
class Rational:
    ...
    def add(self, secondRational: Rational) -> Rational:
        ...
        """
        # Convert both rationals to match the denominators, allowing for addition of numerators.
        n1 = self.numerator * secondRational.denominator
        n2 = secondRational.numerator * self.denominator
        new_num = n1 + n2
        new_denom = self.denominator * secondRational.denominator

        # Instantiates a local Rational with the summed numerators + multiplied denominator
        new_rational = Rational(new_num, new_denom)
        return new_rational
    ...
    def subtract(self, secondRational: Rational) -> Rational:
        """
           ...
           returns the result as a rational number (type Rational)
        """ 
        # Since subtraction is just addition where one values sign is flipped:
        #   Will just flip the sign on the numerator and run self.add with that new value
        negative_rational = Rational(
            numerator=(-1 * secondRational.numerator),
            denominator=secondRational.denominator
        )
        return self.add(negative_rational)
    ...
    def multiply(self, secondRational: Rational) -> Rational:
        """
           ...
           returns the result as a rational number (type Rational)
        """ 
        # Multiply numerators and denominators together to get the return value
        new_denom = self.denominator * secondRational.denominator
        new_num = self.numerator * secondRational.numerator
        new_rational = Rational(new_num, new_denom)
        return new_rational
    ...
    def divide(self, secondRational: Rational) -> Rational:
        """
           ...
           returns the result as a rational number (type Rational)
        """ 
        # Since rational dividing is just multiplying the inverse of the second rational:
        #   Can just flip the second rational and multiply
        new_rational = Rational(
            numerator=secondRational.denominator,
            denominator=secondRational.numerator
        )
        return self.multiply(new_rational)
